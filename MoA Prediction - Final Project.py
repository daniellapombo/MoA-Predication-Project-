...
model = Pipeline([('clf', MultiOutputClassifier(KNeighborsClassifier()))])
...
gs_clf_svm = GridSearchCV(model, parameters, verbose = 1, cv = 5)
gs_clf_svm = gs_clf_svm.fit(x_train_pca, y_train)
print(gs_clf_svm.best_score_)
print(gs_clf_svm.best_params_)
print(gs_clf_svm.best_estimator_)
...
#dict_param = [{'estimator': MultiOutputClassifier(KNeighborsClassifier()), 'grid_param':{'n_neighbors':[3,5,10]}}]
dict_param = [{'estimator': Pipeline([('clf', MultiOutputClassifier(KNeighborsClassifier()))]), 'grid_param':{'clf__estimator__n_neighbors':[3,5,10]}},
             {'estimator': Pipeline([('clf', MultiOutputClassifier(LinearSVM()))]), 'grid_param':{'clf__estimator__C':[3,5,10]}},
             {'estimator': Pipeline([('clf', MultiOutputClassifier(ElasticNet()))]), 'grid_param':{'clf__estimator__alpha':[3,5,10]}}]
